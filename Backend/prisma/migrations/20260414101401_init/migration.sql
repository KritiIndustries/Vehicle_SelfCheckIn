...
CREATE TABLE `REJECTED_VEHICLE` (
    ...
    INDEX `REJECTED_VEHICLE_Driver_Checkin_Id_idx`(`Driver_Checkin_Id`),
    INDEX `REJECTED_VEHICLE_Rejected_By_idx`(`Rejected_By`),
    PRIMARY KEY (`Id`)
) DEFAULT CHARACTER SET utf8mb4 COLLATE utf8mb4_unicode_ci;
...
-- AddForeignKey
ALTER TABLE `REJECTED_VEHICLE` ADD CONSTRAINT `REJECTED_VEHICLE_Driver_Checkin_Id_fkey` FOREIGN KEY (`Driver_Checkin_Id`) REFERENCES `DRIVER_CHECKIN`(`Id`) ON DELETE RESTRICT ON UPDATE CASCADE;

-- AddForeignKey
ALTER TABLE `REJECTED_VEHICLE` ADD CONSTRAINT `REJECTED_VEHICLE_Rejected_By_fkey` FOREIGN KEY (`Rejected_By`) REFERENCES `GUARD_MASTER`(`Guard_Id`) ON DELETE RESTRICT ON UPDATE CASCADE;
